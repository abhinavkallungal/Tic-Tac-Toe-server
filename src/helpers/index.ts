...
export default {
  ...
  checkWin: (player: string, room: any) => {
    ...
    for (i = 0; i < winCombinations.length; i++) {
      ...
      for (j = 0; j < winCombinations[i].length; j++) {
        ...
        if (markCount === 3) {
          db.play = db.play.filter((item) => item.id !== room.id);
          return true;
        }
      }
    }
  ...
  playTurn: (player: string, roomid: string) => {
    ...
    if (player === "firstPlayer") {
      ...
      app.io
        ...
        .emit(
          "message",
          `your oponent ${db.users[secondPlayer].name} is playing`
        );
    }
  },
  ...
  validateRoomIdForSpectat: (roomid: string) => {
    let roomIndex = db.play.findIndex((item) => item.id === roomid);
